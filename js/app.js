    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (const department of response) {
                ...
                $("#m-employeeDepartmentId-txt .t-combobox-data").append(optionHTML);
            }
        }
    });
}
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (const positions of response) {
                // let optionHTML = `<option value="${department.DepartmentId}">${department.DepartmentName}</option>`;
                let optionHTML = `<div class="t-combobox-item" value="${positions.PositionId}">${positions.PositionName}</div>`
                $("#m-employeePositionId-txt .t-combobox-data").append(optionHTML);
            }
        }
    });
}